...
export function TrendChart({ chart, suggestions, user, workouts, reload, onInfoClick }) {
  ...
  const cards = [
    [chart.currentRpe.toFixed(1), chart.cardLabels?.[0] || t("trend.currentValue"), chart.trendColor],
    [chart.avgRpe.toFixed(1), chart.cardLabels?.[1] || t("trend.avgValue"), "var(--text)"],
    [chart.sessionsList.length, chart.cardLabels?.[2] || t("trend.sessions"), "var(--text)"],
  ];
  ...
  return (
    <div style={{ background: "var(--bg2)", border: "1px solid var(--blue)", borderRadius: 16, padding: 20, marginBottom: 20, boxShadow: "0 4px 12px rgba(0,0,0,0.15)" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
        <h4 style={{ fontSize: 14, fontWeight: 700, marginBottom: 0 }}>{chart.title || t("trend.title")}</h4>
      ...
      <TrendSvg chart={chart} boxW={boxW} boxH={boxH} bx={bx} by={by} lastPt={lastPt} />

      <TrendLegend chart={chart} t={t} />

      {showCta && (
        <>
          <button onClick={() => setModalOpen(true)} style={{ display: "block", width: "100%", marginTop: 14, border: "1.5px solid var(--red)", borderRadius: 99, padding: "10px", textAlign: "center", fontSize: 14, fontWeight: 700, color: "var(--red)", background: "transparent", cursor: "pointer", fontFamily: "inherit" }}>
            ↑ {t("trend.increaseWeight")}
          </button>
          <SuggestionsModal open={modalOpen} onClose={() => setModalOpen(false)} suggestions={suggestions} user={user} workouts={workouts} reload={reload} />
        </>
      )}
    </div>
  );
...
function TrendSvg({ chart, boxW, boxH, bx, by, lastPt }) {
  return (
    <svg width="100%" height={chart.H} viewBox={`0 0 ${chart.W} ${chart.H}`} style={{ display: "block", overflow: "visible" }}>
      ...
      {GRID_LEVELS.map((lvl) => {
        ...
        return (
          <g key={lvl}>
            <line x1={0} y1={gy} x2={chart.W} y2={gy} stroke={isT ? "rgba(245,196,0,0.25)" : "rgba(255,255,255,0.05)"} strokeWidth={isT ? 1.5 : 1} strokeDasharray={isT ? "6 4" : "none"} />
            <text x={chart.W - 2} y={gy - 2} fontSize="8" fill={isT ? "rgba(245,196,0,0.6)" : "rgba(136,153,187,0.5)"} textAnchor="end">{lvl}</text>
          </g>
        );
      })}
      ...
      {chart.maPath && <path d={chart.maPath} fill="none" stroke="rgba(136,153,187,0.7)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" strokeDasharray="4 3" />}
  ...
  return (
    <div style={{ display: "flex", gap: 12, marginTop: 8, alignItems: "center" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
        ...
        <span style={{ fontSize: 10, color: "var(--sub)" }}>{t("trend.rpe")}</span>
      </div>
      {chart.maPath && (
        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
          ...
          <span style={{ fontSize: 10, color: "var(--sub)" }}>{t("trend.movingAvg")}</span>
        </div>
      )}
      <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
        <svg width="18" height="6"><line x1="0" y1="3" x2="18" y2="3" stroke="rgba(245,196,0,0.5)" strokeWidth="1.5" strokeDasharray="6 4" /></svg>
        <span style={{ fontSize: 10, color: "var(--sub)" }}>{t("trend.progression")}</span>
      </div>
    </div>
  );
}
