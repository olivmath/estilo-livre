  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent style={{ background: "var(--bg2)", border: "1px solid var(--blue)", borderRadius: 16, color: "var(--text)" }}>
        <DialogHeader>
          <DialogTitle style={{ color: "var(--text)" }}>{t("contasPage.newAccountTitle")}</DialogTitle>
        </DialogHeader>
        <form onSubmit={submit}>
          ...
          <div style={{
            ...
            {t("contasPage.accessHint")}
          </div>
          <Field label={t("contasPage.name")} htmlFor="conta-nome">
            <Input
              id="conta-nome"
              value={form.name}
              onChange={(e) => set("name", e.target.value)}
              placeholder={t("contasPage.namePlaceholder")}
              required
              style={{ background: "var(--bg3)", border: "1px solid var(--blue)", color: "var(--text)" }}
            />
          </Field>
          <Field label={t("contasPage.email")} htmlFor="conta-email">
            <Input
              id="conta-email"
              type="email"
              value={form.email}
              onChange={(e) => set("email", e.target.value)}
              placeholder={t("contasPage.emailPlaceholder")}
              required
              style={{ background: "var(--bg3)", border: "1px solid var(--blue)", color: "var(--text)" }}
            />
          </Field>
          <Field label={t("contasPage.role")} htmlFor="conta-papel">
          ...
          <Button type="submit" disabled={loading} style={{ width: "100%", height: 40 }}>
            {loading ? t("contasPage.creating") : t("contasPage.createAccount")}
          </Button>
        </form>
      </DialogContent>
    </Dialog>
...
export function ContasPage() {
  ...
  const ROLE_LABELS = { admin: t("roles.admin"), professor: t("roles.professor") };
  ...
  async function handleDelete(uid) {
    if (!confirm(t("contasPage.confirmDelete"))) return;
  ...
  return (
    <div style={{ padding: "20px 16px", maxWidth: 700, margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <h1 style={{ fontSize: 22, fontWeight: 800, color: "var(--text)" }}>{t("contasPage.title")}</h1>
        <Button size="sm" onClick={() => setNovoOpen(true)}>
          <Plus size={15} /> {t("contasPage.newAccount")}
        </Button>
      </div>

      {users.length === 0 ? (
        <p style={{ fontSize: 13, color: "var(--sub)" }}>{t("contasPage.noAccounts")}</p>
      ) : (
