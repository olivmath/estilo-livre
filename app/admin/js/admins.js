    ...
    <div class="admin-card">
      ...
      <div class="admin-card-info">
        <div class="admin-card-name">
          ${a.name}
          ${a.email===currentAdmin?.email?'<span class="you-tag">Você</span>':''}
          <span style="font-size:11px; padding:2px 6px; border-radius:4px; margin-left:8px; background:var(--bg3); color:var(--t2)">
            ${a.role === 'admin' ? 'Admin' : 'Professor'}
          </span>
        </div>
...
function openAdminModal() {
  ...
  if (roleEl) roleEl.value = 'professor';
...
function saveAdmin() {
  ...
  const roleEl = document.getElementById('new-admin-role');
  const role = roleEl ? roleEl.value : 'professor';
  ...
  admins[email]={name,email,pw,role};DB.set('admins',admins);closeAdminModal();renderAdmins();
}
